...
class CompactKalmanFilter(object):
    """
    ...
    def forward_step_fn(self, params, inputs):
        ...
        :return:
        """
        mu_pred, Sigma_pred, _, _, _, _, alpha, u, state, buffer, _, _, _ = params
        ...
        # NEW: computing mu_y_t and Sigma_y_t
        mu_y_t = tf.squeeze(tf.matmul(C, tf.expand_dims(mu_t,2)))
        Sigma_y_t = tf.matmul(tf.matmul(C, Sigma_t), C, transpose_b=True) + self.R
        ...
        return mu_pred, Sigma_pred, mu_t, Sigma_t, mu_y_t, Sigma_y_t, alpha, u, state, buffer, A, B, C
    ...
    def compute_forwards(self, reuse=None):
        ...
        dummy_init_mu_y_t = tf.ones([self.Sigma.get_shape()[0], self.dim_y])
        dummy_init_Sigma_y_t = tf.ones([self.Sigma.get_shape()[0], self.dim_y, self.dim_y])
        forward_states = tf.scan(self.forward_step_fn, tf.transpose(inputs, [1, 0, 2]),
                                 initializer=(self.mu, self.Sigma, self.mu, self.Sigma, dummy_init_mu_y_t, dummy_init_Sigma_y_t, alpha, u, state, buffer,
    ...
    def get_elbo(self, forward_states):
        y_masked = tf.multiply(tf.expand_dims(self.mask, 2), self.y)
        mu_y = forward_states[2]
        Sigma_y = forward_states[3]

        y_altered = tf.add(y_masked, tf.multiply(tf.expand_dims(1-self.mask, 2), mu_y))
        
        num_el = tf.reduce_sum(self.mask)
        
        mvn_emision = MultivariateNormalTriL(mu_y, Sigma_y)
        log_prob_emision = mvn_emision.log_prob(y_altered)
        log_prob_emision = tf.multiply(self.mask, log_prob_emision)
        log_prob_emision = tf.truediv( tf.reduce_sum(log_prob_emision), num_el)
        
        return log_prob_emision
     

    def filter(self):
        mu_pred, Sigma_pred, mu_filt, Sigma_filt, mu_y, Sigma_y, alpha, u, state, buffer, A, B, C = forward_states = \
            self.compute_forwards(reuse=True)
        forward_states = [mu_filt, Sigma_filt, mu_y, Sigma_y]
